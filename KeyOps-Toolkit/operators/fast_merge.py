...
class FastMerge(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.selected_verts = [vert for vert in bm.verts if vert.select]

        if len(self.selected_verts) == 1 or not self.prefs.fast_merge_last:
            obj = context.object
            ...
            vertices += self.selected_verts
            vertices = list(set(vertices))

            nearest_vertex = find_nearest_visible_vertex_to_mouse(context, self.mouse_position, vertices, matrix_world)

            if nearest_vertex:
                if nearest_vertex not in self.selected_verts:
                    self.selected_verts.append(nearest_vertex)
                ...
                if self.preserve_uvs:
                    ...
                    bmesh.ops.pointmerge(bm, verts=self.selected_verts, merge_co=nearest_vertex.co)
            ...
            active_edge = selected_history if selected_history in self.selected_verts else []
